...
**Rescate a `develop`.** Este ADR y su cambio de código vivían solo en
`feature/1-configurar-ci-institucional`, la rama del pipeline retirado, y se
habrían perdido al borrarla: en `develop` la configuración `production` de
`angular.json` no declaraba `optimization`, de modo que el inlining seguía
activo y `ng build --configuration production` seguía necesitando internet.
Eso importa más ahora que antes, porque ese build es un paso del job
`admin-web-ci` que `docs/spec/feature-0011-devops-delivery-pipeline/spec.md`
convierte en check **obligatorio**: un paso intermitente que depende de un
tercero no puede ser una compuerta de merge. El runner de GitHub sí tiene
salida a internet, así que el fallo sería esporádico — que es la peor clase
de fallo para una compuerta.
