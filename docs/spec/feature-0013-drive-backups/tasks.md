...
- [x] **T1.2** Configurar carpeta exclusiva y OAuth de la cuenta de backups; probar renovación desatendida y recuperar crypt desde copia offline.
  **Evidencia:** el operador confirmó el bootstrap interactivo y la custodia offline el 2026-09-17; falta la prueba desde Oracle antes de activar timers.
- [x] **T1.3** Configurar checks externos y email; guardar ping URLs como secretos.
  **Evidencia:** Healthchecks autohospedado y checks rotados/validados por el operador el 2026-09-17.
...
- [ ] **T4.1** Instalar timer persistente y lock en Oracle; enlazar /fail y éxito solo después de copia comprobada.
  *Pendiente de instalación operativa y primer backup remoto verificado.*
- [ ] **T4.2** Programar restore mensual aislado en home-server y alerta al vencer 35 días; dump diario vence a 26 horas.
  *Implementación corregida para aislamiento; pendiente de instalación operativa y primer restore remoto real.*
- [ ] **T4.3** Documentar restauración de módulos, permisos y artefactos asociados; medir RPO/RTO.
  *Runbook actualizado; falta registrar los tiempos de un simulacro real.*
- [ ] **T4.4** Ejecutar test-e2e y suite completa antes de PR; no adjuntar dumps al CI público.
  *Pendiente de suite completa fresca y evidencia E2E real sin secretos.*

## Cierre

- [x] **ONB.1** Acompañar alta/selección de cuenta de backups y carpeta backups-hato-erp.
- [x] **ONB.2** Acompañar OAuth, crypt y recuperación offline según PREFLIGHT-PRODUCCION.
- [x] **ONB.2A** Crear cliente OAuth propio de rclone bajo la cuenta de backups y validar consentimiento desatendido.
- [x] **ONB.3** Verificar cuota real, renovación OAuth y email de alertas autohospedadas.
