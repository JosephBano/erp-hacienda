...
- **Guardarraíl estructural del ADR-0018 sec.4:** los cuatro tipos de valor son
...
7. **Endpoints REST** para CRUD de `animal_traits` (sólo lectura por parte del
   ...
   exigencia del `PLAN-FASE-3-4.md` sec.2.2).
...
1. **Invariante estructural: no se puede declarar unidad ni aceptar decimal
   ...
   sec.4 es que **la petición misma** ("quiero un valor numérico libre") es la
...
8. **Idempotencia de sync.** Misma operación push × 3 → una sola fila en
   `trait_observations`. Test obligatorio por `PLAN-FASE-3-4.md` sec.2.2.
...
- El **índice de madre** del sec.4.6 — vive en el macro plan sec.4.6, depende de
  3.5b.5-A y 3.5b.5-B.
...
| `current_disposition` se almacena en una tabla | Implementación en una sola función derivada. Code review explícito del sec.Tarea 6. |
