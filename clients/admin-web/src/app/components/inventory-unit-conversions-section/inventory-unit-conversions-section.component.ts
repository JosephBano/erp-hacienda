...
export class InventoryUnitConversionsSectionComponent {
  ...
  readonly columns: CatalogColumn<InventoryUnitConversionDto>[] = [
    { key: 'fromUnit', label: 'Desde' },
    { key: 'toUnit', label: 'Hacia' },
    { key: 'factor', label: 'Factor' },
  ];
  readonly actions: CatalogAction<InventoryUnitConversionDto>[] = [];
  create(): void {
    if (!this.fromUnit.trim() || !this.toUnit.trim() || this.factor === null || this.factor <= 0) {
      this.errorMessage = 'Completa los datos de la conversión.';
      return;
    }
    this.submitting = true;
    this.api
      .registerInventoryUnitConversion(this.itemId, {
        FromUnit: this.fromUnit.trim(),
        ToUnit: this.toUnit.trim(),
        Factor: this.factor,
      })
      .subscribe({
        next: () => {
        ...
        error: () => {
          this.submitting = false;
          this.errorMessage = 'No se pudo crear la conversión.';
        },
      });
  }
}
