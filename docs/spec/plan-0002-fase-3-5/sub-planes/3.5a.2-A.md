# 3.5a.2-A — Catálogos, payload y forward-compat del tratamiento

> **Sub-plan de la Fase 3.5, sec.3.5a.2** — extraído del antiguo plan único de la
> fase, hoy [`spec-3.5a.md`](../spec-3.5a.md) sec.3.5a.2. Donde este documento dice "macro plan"
...
- **Pares del split:**
  - [`3.5a.2-B`](./3.5a.2-B.md) (rama `feature/livestock-treatment-dose-logic`): las tres formas de dosis, calculada vs. administrada, opcionalidad, observación libre y `TreatmentCourse`.
  - [`3.5a.2-C`](./3.5a.2-C.md) (rama `feature/field-app-treatment-ui`): vacunación como camino separado y pantalla de campo con vía y motivo en la misma pasada.
- **Fuente original:** [`spec-3.5a.md` sec.3.5a.2](../spec-3.5a.md#35a2--featurelivestock-treatment-detail--estructural--split-en-35a2-a-35a2-b-y-35a2-c)
...
- **El choque con la Constitución es aparente.** Asumir que estructurar la dosis
  ...
  sub-rama [3.5a.2-B](./3.5a.2-B.md) (donde el operario
...
4. **`Disposal.Cause` y `GroupMortality.Cause`** ya los introduce [3.5a.3](../spec-3.5a.md#35a3--featurelivestock-mortality-causes)
...
5. **Migración EF Core:** tablas nuevas `administration_routes` y
   ...
   historia en datos legacy (mismo patrón que [`fase-3/spec.md` Bloque 3.A](../../fase-3/spec.md#bloque-3a--suelo-en-el-backend-4-ramas)).
