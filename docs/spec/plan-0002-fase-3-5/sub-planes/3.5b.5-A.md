# 3.5b.5-A — Núcleo de AnimalTrait y TraitObservation
...
- **Pares del split:**
  - [`3.5b.5-B`](./3.5b.5-B.md) (rama `feature/livestock-selection-criterion-deprecation`): absorbe `SelectionCriterion` y depreca la tabla paralela.
  - [`3.5b.5-C`](./3.5b.5-C.md) (rama `feature/livestock-trait-alerts-and-versioning`): advertencias visibles en la ficha + versionado de definiciones usadas.
...
4. **Tabla `trait_observations`** (registro fechado y firmado):
   ...
   **`trait_version_at_observation` es la columna que el versionado de
   [3.5b.5-C](./3.5b.5-C.md) explota.** Se setea en el
...
7. **Endpoints REST** para CRUD de `animal_traits` (sólo lectura por parte del
   ...
   exigencia de pruebas por capa de [`PROTOCOLO-DE-TRABAJO.md`](../../../PROTOCOLO-DE-TRABAJO.md)).
...
8. **Idempotencia de sync.** Misma operación push × 3 → una sola fila en
   `trait_observations`. Test obligatorio por las pruebas por capa de [`PROTOCOLO-DE-TRABAJO.md`](../../../PROTOCOLO-DE-TRABAJO.md).
...
- La evaluación **funcional de futuras madres** (sesión con captura masiva de
  ...
  posterior a [3.5b.5-B](./3.5b.5-B.md).
